...
@Repository
public class PraiseRepositoryImpl extends GenericRepositoryImpl<PraiseInfo,Long> implements PraiseRepository {

	@SuppressWarnings("unchecked")
	@Override
	public List<PraiseInfo> findPraiseTopList(final String siteId,final String columnId,final boolean isOrgan,
			final String startTime,final String endTime,final Pager pager){
		String sql="select t.PID,t.COUNT,t.LINK,t.TITLE,t.ORGAN_ID as userName,t.VIRTUAL_COUNT as virtualCount,count(h.PID) as ps from t_praise t inner join t_praise_history h on t.PID=h.PID ";
		...
		try {
			PropertyDescriptor[] props = Introspector
					.getBeanInfo(PraiseInfo.class)
					.getPropertyDescriptors();

			for (Map<String, Object> map : list) {
				PraiseInfo t = PraiseInfo.class.newInstance();
	...
	@Override
	public Long findPraiseTopListCount(final String siteId,final String columnId,final boolean isOrgan,
			final String startTime,final String endTime){
		String sql="select t.* from t_praise t inner join t_praise_history h on t.PID=h.PID ";
		if(StringUtils.isNotEmpty(siteId)){
			sql+="  and t.SITE_ID in ("+siteId+")";
		}
		if(StringUtils.isNotEmpty(columnId)){
		...
		Query query= getSession().createSQLQuery(sql).addEntity(PraiseInfo.class);
		return (long) query.list().size();
	}
	
	@Override
	public Long findPraiseTopCountByPid(final Long pid,final String startTime,final String endTime){
		String sql="select count(*) from PraiseHistory h where h.pid=? ";
		if(StringUtils.isNotEmpty(startTime)){
	...
	@Override
	public Long findPraiseCountByOrgan(final Long userId) {
		String sql="select sum(count) from t_praise p where " +
	...
	@Override
	public Long findPraiseByRecordId(final Long recordId) {
		String sql="select sum(count) from t_praise p where record_id="+recordId;
		Query query= getSession().createSQLQuery(sql);
		return ((BigDecimal)query.uniqueResult()).longValue();
	}
